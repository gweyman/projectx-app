...
export function processSessionReport(athleteState, sessionReport) {
  ...
  // Hard rule (v3 §8.2): 5–6 soreness for 2 consecutive slots forces OFF
const updatedSorenessScores = [...(athleteState.recentSorenessScores ?? []), sessionReport.soreness];
const forcedOff = checkConsecutiveModerate(updatedSorenessScores);
if (forcedOff) {
  messages.push(MESSAGES.FORCED_OFF_CONSECUTIVE_MODERATE);
  flags.push({ type: 'forced_off', reason: 'consecutive_moderate_soreness' });
 }
